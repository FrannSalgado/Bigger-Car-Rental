...
describe('AuthController (e2e) - Simulated Endpoints', () => {
    ...
    it('/auth/forgot-password (POST) should return generic message when email is not found', async () => {
        ...
        expect(response.body).toEqual({
            message: 'If the email is valid, you will receive an email with instructions.',
        });
    ...
    it('/auth/forgot-password (POST) should simulate forgot password when user exists', async () => {
        mockUsersService.searchByEmail.mockResolvedValueOnce({ email: 'existing@example.com' });
        ...
        expect(mockJwtTokenService.generateResetToken).toHaveBeenCalledWith('existing@example.com');
